...
class Router {
  public:
    ...
    std::optional<MatchParams> at(const std::string &path) const { return at(path.c_str(), path.size()); }
  ...
  private:
    ...
    bool match_recursive(const Node &node, const char *path, size_t len, size_t pos, MatchParams &result) const;
};
