...
class WsRingBuffer {
  public:
    ...
    explicit WsRingBuffer(size_t initial_capacity = DEFAULT_CAPACITY, size_t max_capacity = MAX_CAPACITY);
    ~WsRingBuffer();
    ...
    size_t available() const { return capacity_ - (write_pos_ - read_pos_); }
    ...
    bool full() const { return (write_pos_ - read_pos_) >= capacity_; }
...
struct WsConnectionState {
    ...
    ~WsConnectionState() { free(output_pending); }
