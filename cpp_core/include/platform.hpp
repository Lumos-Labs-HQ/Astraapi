...
#else
inline void fast_memcpy_small(void *dst, const void *src, size_t len) { memcpy(dst, src, len); }
#endif
...
inline ssize_t platform_socket_writev(int fd, PlatformIoVec *iov, int iovcnt) {
    ...
    WSABUF *bufs =
        (iovcnt <= 64) ? stack_bufs : static_cast<WSABUF *>(malloc(static_cast<size_t>(iovcnt) * sizeof(WSABUF)));
    if (!bufs) return -1;
    ...
    DWORD bytes_sent = 0;
    int result = WSASend(static_cast<SOCKET>(fd), bufs, iovcnt, &bytes_sent, 0, nullptr, nullptr);
...
inline ssize_t platform_socket_write(int fd, const void *buf, size_t len) { return send(fd, buf, len, MSG_NOSIGNAL); }
...
inline ssize_t platform_socket_writev(int fd, PlatformIoVec *iov, int iovcnt) {
    static_assert(sizeof(PlatformIoVec) == sizeof(struct iovec), "PlatformIoVec must match struct iovec layout");
