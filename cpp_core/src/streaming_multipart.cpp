...
StreamingMultipartParser::StreamingMultipartParser(const std::string &boundary, size_t max_body_size)
...
FeedResult StreamingMultipartParser::feed(const uint8_t *data, size_t len) {
    ...
    while (pos < len) {
        ...
        switch (state_) {
        case MultipartState::PREAMBLE:
            r = process_preamble(data + pos, len - pos, consumed);
            break;
        case MultipartState::PART_HEADER:
            r = process_header(data + pos, len - pos, consumed);
            break;
        case MultipartState::PART_DATA:
            r = process_data(data + pos, len - pos, consumed);
            break;
        default:
...
FeedResult StreamingMultipartParser::process_preamble(const uint8_t *data, size_t len, size_t &consumed) {
...
FeedResult StreamingMultipartParser::process_header(const uint8_t *data, size_t len, size_t &consumed) {
...
FeedResult StreamingMultipartParser::process_data(const uint8_t *data, size_t len, size_t &consumed) {
    std::string delim = "\r\n" + boundary_;
    ...
    if (len >= delim_len) {
        ...
        while (p < end) {
            ...
            if (!p) break;
            if (memcmp(p, delim_data, delim_len) == 0) {
                pos = p - data;
                break;
            }
            p++;
...
PyObject *StreamingMultipartParser::build_kwargs(PyObject *existing_kwargs) const {
    ...
    for (const auto &file : files) {
        ...
        if (file_size < 1048576) {
            ...
            PyRef data_bytes(PyBytes_FromStringAndSize((const char *)file.data_buf.data(), file_size));
            if (!data_bytes) {
                if (!existing_kwargs) Py_DECREF(kwargs);
                return nullptr;
            }
            ...
            snprintf(tmppath, sizeof(tmppath), "%s/astraapi_upload_%d_%ld.tmp", tmpdir.string().c_str(), counter_val,
                (long)getpid());
            FILE *fp = std::fopen(tmppath, "wb+");
            if (fp) {
                // Write all data via C fwrite() — no Python GIL overhead
                std::fwrite(file.data_buf.data(), 1, file.data_buf.size(), fp);
                std::fclose(fp);
                ...
                if (path_str && mode_str) {
                    ...
                    if (builtins) {
                        ...
                        if (open_fn) {
                            file_obj =
                                PyObject_CallFunctionObjArgs(open_fn.get(), path_str.get(), mode_str.get(), nullptr);
                        }
                    }
                ...
                if (!file_obj) {
                    PyRef data_bytes(PyBytes_FromStringAndSize((const char *)file.data_buf.data(), file_size));
                    if (data_bytes) file_obj = PyObject_CallOneArg(s_bytes_io_cls_, data_bytes.get());
                }
            } else {
...
static void streaming_parser_destructor(PyObject *cap) {
    auto *p = static_cast<StreamingMultipartParser *>(PyCapsule_GetPointer(cap, "streaming_multipart_parser"));
    delete p;
}
...
PyObject *py_create_streaming_multipart_parser(PyObject * /*self*/, PyObject *args) {
    const char *boundary;
    Py_ssize_t blen;
    Py_ssize_t max_body = 0;
    if (!PyArg_ParseTuple(args, "s#|n", &boundary, &blen, &max_body)) return nullptr;
    auto *parser = new StreamingMultipartParser(std::string(boundary, blen), (size_t)std::max((Py_ssize_t)0, max_body));
...
PyObject *py_feed_streaming_multipart(PyObject * /*self*/, PyObject *args) {
    ...
    auto *parser = static_cast<StreamingMultipartParser *>(PyCapsule_GetPointer(cap, "streaming_multipart_parser"));
    if (!parser) {
...
PyObject *py_get_streaming_multipart_form_data(PyObject * /*self*/, PyObject *cap) {
    ...
    if (!fd) {
        PyErr_SetString(PyExc_RuntimeError, "Failed to build FormData");
        return nullptr;
    }
    return fd;
}
