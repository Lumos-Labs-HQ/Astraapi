...
int ws_parse_frame(const uint8_t *data, size_t len, WsFrame *out, bool require_mask, bool allow_rsv) {
...
int WsFragmentAssembler::feed(const WsFrame &frame, const uint8_t *unmasked_payload) {
    ...
    if (opcode == WS_CONTINUATION) {
        ...
        accumulated.insert(accumulated.end(), unmasked_payload, unmasked_payload + frame.payload_len);
...
void ws_parse_frames_nogil(uint8_t *data, size_t data_len, ParseResult &result, WsFragmentAssembler *assembler) {
    ...
    while (total_consumed < data_len) {
        ...
        if (opcode == WS_CLOSE) {
            result.frames.push_back({opcode, (size_t)(frame.payload - data), (size_t)frame.payload_len, false});
            total_consumed += (size_t)consumed;
            break;
        }
        ...
        if (assembler && (opcode == WS_CONTINUATION || !frame.fin)) {
            ...
            if (asm_result == 1 && opcode == WS_CONTINUATION) {
                ...
                result.assembled.push_back({assembler->original_opcode, std::move(assembler->accumulated)});
                assembler->reset();
            }
...
PyObject *py_ws_parse_frames(PyObject * /*self*/, PyObject *arg) {
    ...
    Py_BEGIN_ALLOW_THREADS ws_parse_frames_nogil(data, data_len, pres);
    ...
    for (size_t i = 0; i < pres.frames.size(); i++) {
        ...
        PyObject *payload_bytes =
            PyBytes_FromStringAndSize((const char *)(data + fr.payload_offset), (Py_ssize_t)fr.payload_len);
        if (!payload_bytes) {
            PyBuffer_Release(&buf);
            return nullptr;
        }
...
PyObject *py_ws_build_frame_bytes(PyObject * /*self*/, PyObject *const *args, Py_ssize_t nargs) {
    ...
    if (PyObject_GetBuffer(args[1], &payload_buf, PyBUF_SIMPLE) < 0) return nullptr;
    ...
    if (payload_len > 1024) {
        ...
        Py_BEGIN_ALLOW_THREADS ws_write_frame_header(tmp, (WsOpcode)opcode, payload_len);
...
PyObject *py_ws_build_ping_frame(PyObject * /*self*/, PyObject *arg) {
    ...
    if (arg != Py_None) {
        if (PyObject_GetBuffer(arg, &payload_buf, PyBUF_SIMPLE) < 0) return nullptr;
...
PyObject *py_ws_parse_frames_text(PyObject * /*self*/, PyObject *arg) {
    ...
    for (size_t i = 0; i < pres.frames.size(); i++) {
        ...
        if (fr.opcode == WS_TEXT) {
            payload_obj = PyUnicode_DecodeUTF8(payload_ptr, (Py_ssize_t)fr.payload_len, "surrogateescape");
            if (!payload_obj) {
...
void ws_echo_frames_nogil(uint8_t *data, size_t data_len, std::vector<uint8_t> &out, EchoResult &result) {
    size_t total_consumed = 0;
...
size_t ws_echo_frames_into_buffer(uint8_t *out_buf, size_t out_cap, uint8_t *data, size_t data_len,
    EchoResult &result) {
    size_t total_consumed = 0;
    size_t out_pos = 0;
...
PyObject *py_ws_echo_frames(PyObject * /*self*/, PyObject *arg) {
    ...
    Py_BEGIN_ALLOW_THREADS ws_echo_frames_nogil(data, data_len, out, eres);
    Py_END_ALLOW_THREADS
    ...
    PyObject *close_obj;
    if (eres.has_close) {
        close_obj = PyBytes_FromStringAndSize((const char *)(data + eres.close_payload_offset),
...
bool WsDeflateContext::init() {
    ...
    if (deflateInit2(def, 1, Z_DEFLATED, -server_max_window_bits, 8, Z_DEFAULT_STRATEGY) != Z_OK) {
        inflateEnd(inf);
        delete inf;
        inflate_ctx = nullptr;
        delete def;
...
bool WsDeflateContext::decompress(const uint8_t *in, size_t in_len, std::vector<uint8_t> &out) {
...
bool WsDeflateContext::compress(const uint8_t *in, size_t in_len, std::vector<uint8_t> &out) {
    ...
    if (out.size() >= 4 && out[out.size() - 4] == 0x00 && out[out.size() - 3] == 0x00 && out[out.size() - 2] == 0xFF &&
        out[out.size() - 1] == 0xFF) {
...
class SharedDeflatePool {
  ...
  public:
    ...
    ~SharedDeflatePool() {
        for (auto &e : entries_) {
            if (e.inflate_ctx) {
                inflateEnd(e.inflate_ctx);
                delete e.inflate_ctx;
            }
            if (e.deflate_ctx) {
                deflateEnd(e.deflate_ctx);
                delete e.deflate_ctx;
            }
        }
    }
...
void shared_deflate_pool_release(int idx) { s_deflate_pool.release(idx); }

std::string ws_build_upgrade_response_ext(const char *sec_key, size_t key_len, const char *extensions, size_t ext_len,
    const char *subprotocol, size_t sub_len, WsDeflateContext *deflate_out) {
    ...
    if (negotiate_deflate) {
        ...
        if (deflate_out->server_no_context_takeover) resp += "; server_no_context_takeover";
        if (deflate_out->client_no_context_takeover) resp += "; client_no_context_takeover";
        resp += "\r\n";
    }
