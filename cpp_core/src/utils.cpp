...
PyObject *py_batch_extract_params(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO", (char **)kwlist, &parsed_query, &field_specs)) return nullptr;
...
PyObject *py_batch_coerce_scalars(PyObject *self, PyObject *args) {
    ...
    for (Py_ssize_t i = 0; i < nspecs; i++) {
        ...
        switch (tt) {
        case 1:
            coerced = coerce_to_int(s, slen);
            break; // TYPE_INT
        case 2:
            coerced = coerce_to_float(s, slen);
            break; // TYPE_FLOAT
        case 3:
            coerced = coerce_to_bool(s, slen);
            break; // TYPE_BOOL
        default:
            continue;
        }
