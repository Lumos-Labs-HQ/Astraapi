...
static inline PyObject *make_frame_tuple(uint8_t opcode, PyObject *payload_obj) {
    ...
    switch (opcode) {
    case WS_TEXT:
        opcode_obj = g_opcode_text;
        break;
    case WS_BINARY:
        opcode_obj = g_opcode_binary;
        break;
    case WS_CLOSE:
        opcode_obj = g_opcode_close;
        break;
    default:
        opcode_obj = PyLong_FromLong(opcode);
        if (!opcode_obj) {
    ...
    if (!tuple) {
        if (opcode != WS_TEXT && opcode != WS_BINARY && opcode != WS_CLOSE) Py_DECREF(opcode_obj);
        Py_DECREF(payload_obj);
        return nullptr;
    }
...
WsRingBuffer::WsRingBuffer(size_t initial_capacity, size_t max_capacity) : max_capacity_(max_capacity) {
    capacity_ = 1;
...
WsRingBuffer::~WsRingBuffer() { free_aligned(buffer_); }
...
PyObject *py_ws_echo_direct(PyObject * /*self*/, PyObject *args) {
    ...
    bool appended = state->ring.append(static_cast<const uint8_t *>(py_buf.buf), py_buf.len);
    PyBuffer_Release(&py_buf);
    ...
    Py_BEGIN_ALLOW_THREADS actual_echo_size = ws_echo_frames_into_buffer(echo_buf, max_echo_size, data, data_len, eres);
...
PyObject *py_ws_echo_direct_fd(PyObject * /*self*/, PyObject *args) {
    ...
    auto do_echo = [&]() {
        while (total_consumed < data_len && iov_count < MAX_WRITEV_FRAMES * 2 - 1) {
            ...
            size_t hdr_len = ws_write_frame_header(tl_hdr_buf + hdr_offset, resp_opcode, (size_t)frame.payload_len);
    ...
    Py_BEGIN_ALLOW_THREADS do_echo();
    Py_END_ALLOW_THREADS
        ...
        if (!write_ok && remaining_iovs > 0) {
            ...
            for (int i = 0; i < remaining_iovs; i++) fallback_size += iov_ptr[i].len;
    ...
    if (eres.has_close) {
        PyObject *close_result = PyBytes_FromStringAndSize((const char *)(data + eres.close_payload_offset),
            (Py_ssize_t)eres.close_payload_len);
        if (eres.total_consumed > 0) {
            state->ring.consume(eres.total_consumed);
        }
...
PyObject *py_ws_echo_direct_fd_v2(PyObject * /*self*/, PyObject *args) {
    ...
    auto do_echo = [&]() {
        ...
        if (state->output_pending_len > 0) {
            ssize_t n = platform_socket_write(fd, state->output_pending, state->output_pending_len);
            if (n <= 0) {
                write_ok = false;
                write_eagain = true;
            ...
            if ((size_t)n < state->output_pending_len) {
                ...
                memmove(state->output_pending, state->output_pending + n, rem);
    ...
    // Handle close frame
    if (eres.has_close) {
        PyObject *close_bytes = PyBytes_FromStringAndSize((const char *)(data + eres.close_payload_offset),
            (Py_ssize_t)eres.close_payload_len);
        if (eres.total_consumed > 0) {
            state->ring.consume(eres.total_consumed);
        }
...
PyObject *py_ws_flush_pending(PyObject * /*self*/, PyObject *args) {
    ...
    ssize_t n;
    Py_BEGIN_ALLOW_THREADS n = platform_socket_write(fd, state->output_pending, state->output_pending_len);
    Py_END_ALLOW_THREADS
...
PyObject *py_ws_handle_direct(PyObject * /*self*/, PyObject *args) {
    ...
    Py_BEGIN_ALLOW_THREADS ws_parse_frames_nogil(data, data_len, pres, &state->assembler);
    Py_END_ALLOW_THREADS
        ...
        PyObject *payload_obj = PyBytes_FromStringAndSize((const char *)close_payload, 2);
    ...
    for (size_t i = 0; i < pres.assembled.size(); i++) {
        ...
        if (msg.opcode == WS_TEXT) {
            payload_obj = PyUnicode_DecodeUTF8(payload_ptr, payload_len, "surrogateescape");
            if (!payload_obj) {
                PyErr_Clear();
...
PyObject *py_ws_handle_and_feed(PyObject * /*self*/, PyObject *const *args, Py_ssize_t nargs) {
    ...
    Py_buffer py_buf;
    if (PyObject_GetBuffer(args[1], &py_buf, PyBUF_SIMPLE) < 0) return nullptr;
    ...
    for (size_t i = 0; i < pres.frames.size(); i++) {
        ...
        if (fr.opcode == WS_CLOSE) {
            close_payload_obj = PyBytes_FromStringAndSize(payload_ptr, (Py_ssize_t)fr.payload_len);
            if (!close_payload_obj) return nullptr;
            // Feed close to waiter or buffer
            PyRef tuple(PyTuple_New(2));
            if (!tuple) {
                Py_DECREF(close_payload_obj);
                return nullptr;
            }
...
PyObject *py_ws_handle_json_direct(PyObject * /*self*/, PyObject *args) {
    ...
    for (size_t i = 0; i < pres.frames.size(); i++) {
        ...
        if (fr.opcode == WS_TEXT && fr.payload_len > 0) {
            payload_obj = yyjson_parse_to_pyobject(payload_ptr, (size_t)fr.payload_len);
            if (!payload_obj) {
                PyErr_Clear();
                payload_obj = PyUnicode_DecodeUTF8(payload_ptr, (Py_ssize_t)fr.payload_len, "surrogateescape");
            }
    ...
    for (size_t i = 0; i < pres.assembled.size(); i++) {
        ...
        if (msg.opcode == WS_TEXT && payload_len > 0) {
            payload_obj = yyjson_parse_to_pyobject(payload_ptr, (size_t)payload_len);
            if (!payload_obj) {
                PyErr_Clear();
                payload_obj = PyUnicode_DecodeUTF8(payload_ptr, payload_len, "surrogateescape");
            }
...
PyObject *py_ws_get_metrics(PyObject * /*self*/, PyObject *capsule) {
    ...
    return Py_BuildValue("(KKKK)", (unsigned long long)state->messages_in, (unsigned long long)state->bytes_in,
        (unsigned long long)state->messages_out, (unsigned long long)state->bytes_out);
}
