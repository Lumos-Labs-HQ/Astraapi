...
static PyObject *val_to_pyobject(yyjson_val *val) {
    ...
    case YYJSON_TYPE_NUM: {
        ...
        if (subtype == YYJSON_SUBTYPE_UINT) return PyLong_FromUnsignedLongLong(yyjson_get_uint(val));
        if (subtype == YYJSON_SUBTYPE_SINT) return PyLong_FromLongLong(yyjson_get_sint(val));
        // YYJSON_SUBTYPE_REAL
    ...
    case YYJSON_TYPE_OBJ: {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            if (PyDict_SetItem(dict.get(), py_key.get(), py_val.get()) < 0) return nullptr;
        }
        return dict.release();
    }
...
PyObject *yyjson_parse_to_pyobject(const char *data, size_t len) {
    ...
    if (!doc) {
        PyErr_Format(PyExc_ValueError, "JSON parse error at position %zu: %s", err.pos,
