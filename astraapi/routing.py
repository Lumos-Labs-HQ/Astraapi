...
from astraapi.dependencies.models import Dependant
...
from astraapi.utils import (
    create_model_field,
    generate_unique_id,
    get_value_or_default,
    is_body_allowed_for_status_code,
)
from astraapi import _routing_base as routing
...
from astraapi._response import JSONResponse, Response
from astraapi._routing_base import (
    BaseRoute,
    Match,
    compile_path,
    get_name,
)
from astraapi._routing_base import Mount as Mount  # noqa
from astraapi._types import AppType, ASGIApp, Lifespan, Receive, Scope, Send
from astraapi._websocket import WebSocket
from typing_extensions import deprecated

from astraapi._datastructures_impl import FormData, Headers, UploadFile
...
class _DefaultLifespan:
    ...
    in Starlette. AstraAPI keeps it to maintain backward compatibility with
...
def get_request_handler(
    ...
    async def app(request: Request) -> Response:
        ...
        file_stack = request.scope.get("astraapi_middleware_astack")
        assert isinstance(file_stack, AsyncExitStack), (
            "astraapi_middleware_astack not found in request scope"
        )
        ...
        errors: list[Any] = []
        async_exit_stack = request.scope.get("astraapi_inner_astack")
        assert isinstance(async_exit_stack, AsyncExitStack), (
            "astraapi_inner_astack not found in request scope"
        )
...
def get_websocket_app(
    ...
    async def app(websocket: WebSocket) -> None:
        ...
        async_exit_stack = websocket.scope.get("astraapi_inner_astack")
        assert isinstance(async_exit_stack, AsyncExitStack), (
            "astraapi_inner_astack not found in request scope"
        )
        solved_result = await solve_dependencies(
...
def _build_field_specs(
    ...
    from astraapi._compat import is_sequence_field, get_cached_model_fields
    from astraapi.dependencies.utils import _get_scalar_type_tag, _is_json_field, get_validation_alias
...
def _make_param_validator(dependant: Dependant) -> Optional[Any]:
    ...
    """
    from pydantic import BaseModel as _BaseModel
    from astraapi._compat import get_cached_model_fields
    from astraapi.dependencies.utils import get_validation_alias
    ...
    for loc, fields in [
        ...
        for field in fields:
            if lenient_issubclass(field.type_, _BaseModel):
                ...
                for inner_f in get_cached_model_fields(field.type_):
                    inner_alias = get_validation_alias(inner_f)
                    from astraapi.dependencies.utils import is_sequence_field as _isf
    ...
    def _validate(kwargs_dict: dict) -> tuple[dict, list]:
        ...
        for _mpi in model_param_info:
            ...
            if loc == "query":
                ...
                if not _cookie_hdrs:
                    try:
                        from astraapi._cpp_server import _current_raw_headers as _crh3
                ...
                if not _raw_hdrs:
                    try:
                        from astraapi._cpp_server import _current_raw_headers as _crh2
                        _raw_hdrs = _crh2.get()
                    except Exception: pass
                if _raw_hdrs:
...
def _flatten_deps(
    ...
    Uses cache_key (call + scopes) for deduplication, matching AstraAPI semantics:
    ...
    for i, sub_dep in enumerate(dependant.dependencies):
        ...
        from astraapi.security.http import HTTPBase
...
def _make_lightweight_request(raw_headers, method, path, app=None, route_id=None, _shim_route=None):
    ...
    """
    # Use the AstraAPI Request class (already imported at module level).
    ...
    _root_path = ''
    try:
        from astraapi._cpp_server import _server_root_path as _srp
        _root_path = _srp
    except Exception:
        pass
    ...
    if _route is None:
        try:
            from astraapi._cpp_server import _current_route as _cr
            _route = _cr.get()
        except Exception:
            pass
        ...
        if self.response_model:
            ...
            if annotation_is_pydantic_v1(self.response_model):
                raise PydanticV1NotSupportedError(
                    "pydantic.v1 models are no longer supported by AstraAPI."
...
class APIRouter(routing.Router):
    ...
    an app in multiple files. It would then be included in the `AstraAPI` app, or
    in another `APIRouter` (ultimately included in the app).

    Read more about it in the
    [AstraAPI docs for Bigger Applications - Multiple Files](https://astraapi.tiangolo.com/tutorial/bigger-applications/).
    ...
    def __init__(
        ...
        *,
        ...
        dependency_overrides_provider: Annotated[
            ...
            Doc(
                """
                Only used internally by AstraAPI to handle dependency overrides.

                You shouldn't need to use it. It normally points to the `AstraAPI` app
                object.
                """
            ),
        ] = None,
        route_class: Annotated[
            ...
            Doc(
                ...
                Read more about it in the
                [AstraAPI docs for Custom Request and APIRoute class](https://astraapi.tiangolo.com/how-to/custom-request-and-route/#custom-apiroute-class-in-a-router).
                """
            ),
        ] = APIRoute,
    ...
    def websocket(
        ...
        **Example**
        ...
        ```python
        from astraapi import APIRouter, AstraAPI, WebSocket
