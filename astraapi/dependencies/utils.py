...
from astraapi.background import BackgroundTasks
from astraapi.concurrency import (
    asynccontextmanager,
    contextmanager_in_threadpool,
)
from astraapi.dependencies.models import Dependant
from astraapi.exceptions import DependencyScopeError
from astraapi.logger import logger
from astraapi.security.oauth2 import SecurityScopes
from astraapi.types import DependencyCacheKey
from astraapi.utils import create_model_field, get_path_param_names
from pydantic import BaseModel, Json
from pydantic.fields import FieldInfo
from astraapi._background import BackgroundTasks as NativeBackgroundTasks
from astraapi._concurrency import run_in_threadpool
from astraapi._datastructures_impl import (
    FormData,
    Headers,
    ImmutableMultiDict,
    QueryParams,
...
from astraapi._request import HTTPConnection, Request
...
def get_flat_dependant(
    ...
    *,
    ...
    use_parent_oauth_scopes = (dependant.own_oauth_scopes or []) + (
        parent_oauth_scopes or []
    )
...
def _get_signature(call: Callable[..., Any]) -> inspect.Signature:
    if sys.version_info >= (3, 10):
        try:
            ...
            # Handle type annotations with if TYPE_CHECKING, not used by AstraAPI
    ...
    for param_name, param in signature_params.items():
        ...
        if add_non_field_param_to_dependency(
            ...
            assert param_details.field is None, (
                f"Cannot specify multiple AstraAPI annotations for {param_name!r}"
            )
            continue
...
def analyze_param(
    *,
    ...
    if get_origin(use_annotation) is Annotated:
        ...
        astraapi_annotations = [
            arg
            for arg in annotated_args[1:]
            if isinstance(arg, (FieldInfo, params.Depends))
        ]
        astraapi_specific_annotations = [
            arg
            for arg in astraapi_annotations
            if isinstance(
                arg,
                (
                    params.Param,
        ...
        if astraapi_specific_annotations:
            astraapi_annotation: Union[FieldInfo, params.Depends, None] = (
                astraapi_specific_annotations[-1]
            )
        else:
            astraapi_annotation = None
        # Set default for Annotated FieldInfo
        if isinstance(astraapi_annotation, FieldInfo):
            # Copy `field_info` because we mutate `field_info.default` below.
            field_info = copy_field_info(
                field_info=astraapi_annotation,
        ...
        elif isinstance(astraapi_annotation, params.Depends):
            depends = astraapi_annotation
    ...
    # Get FieldInfo from default value
        ...
        assert field_info is None, (
            "Cannot specify AstraAPI annotations in `Annotated` and default value"
            f" together for {param_name!r}"
        )
        field_info = value
    ...
    if lenient_issubclass(
        ...
        if depends is None:
            assert field_info is None, (
                f"Cannot specify AstraAPI annotation for type {type_annotation!r}"
            )
...
async def solve_dependencies(
    *,
    ...
    request_astack = request.scope.get("astraapi_inner_astack")
    assert isinstance(request_astack, AsyncExitStack), (
        "astraapi_inner_astack not found in request scope"
    )
    function_astack = request.scope.get("astraapi_function_astack")
    assert isinstance(function_astack, AsyncExitStack), (
        "astraapi_function_astack not found in request scope"
    )
    values: dict[str, Any] = {}
    errors: list[Any] = []
    if response is None:
...
def is_union_of_base_models(field_type: Any) -> bool:
    ...
    from astraapi.types import UnionType
