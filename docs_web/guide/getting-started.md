...
If you have used FastAPI, you already know AstraAPI. The same decorators (`@app.get`, `@app.post`), the same Pydantic models, the same dependency injection system, and the same automatic OpenAPI documentation. The only differences? Your endpoints run dramatically faster, and you do not need gunicorn or uvicorn.
...
| Server required | uvicorn / gunicorn | **Built-in multi-worker** — none needed |
...
## When FastAPI Is Still Great

AstraAPI does not replace FastAPI — it complements it. If your app is I/O-bound (heavy database workloads, external API calls), the C++ core speedup may be less noticeable because you are waiting on network. But for:
...
Notice something? **No uvicorn import. No server configuration. Just `app.run()`.**
